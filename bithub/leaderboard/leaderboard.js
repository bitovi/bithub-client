steal('can/control', './leaderboard.mustache', function(Control, initView){
	...
	return Control.extend({
		init : function(){
			...
			this.__updatedUsers = this.options.users.attr('length') > 0;
		},
		...
		replaceUsers : function(el, ev){
			var val = el.is(':checked'),
				users = val ? this.options.users : filterAdmins(this.options.users),
				self = this,
				appendUsers = function(){
					var length = self.users.attr('length');
					self.users.push.apply(self.users, users.slice(length, length + 20));
